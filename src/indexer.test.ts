...
describe("ConditionalOrderCreated Handler", () => {
  ...
  it("should create a ConditionalOrder entity from a TWAP event", async () => {
    ...
    const orders = await indexer.ConditionalOrder.getAll();
  ...
  it("should decode TWAP params correctly", async () => {
    ...
    const order = (await indexer.ConditionalOrder.getAll())[0]!;
  ...
  it("should resolve COWShed proxy owner if proxy exists", async () => {
    ...
    // COWShedBuilt first (creates the OwnerMapping), then the order
    await indexer.process({
      chains: {
        1: {
          simulate: [
...
describe("MerkleRootSet Handler", () => {
  it("should create a MerkleRoot entity", async () => {
    ...
    const roots = await indexer.MerkleRoot.getAll();
  ...
  it("should handle LOG proof location", async () => {
          ...
          simulate: [
            {
              ...
              params: {
                ...
                proof: { location: 1n, data: "0xdeadbeef" }, // LOG proof
              },
            },
          ],
        },
    ...
    const root = (await indexer.MerkleRoot.getAll())[0]!;
...
describe("COWShedBuilt Handler", () => {
  it("should create a OwnerMapping entity mapping proxy to EOA", async () => {
    ...
    const proxies = await indexer.OwnerMapping.getAll();
  ...
  it("should use proxyAddress-chainId as entity ID", async () => {
    ...
    const proxy = (await indexer.OwnerMapping.getAll())[0]!;
  ...
  it("should retroactively update ConditionalOrders owned by the proxy", async () => {
    const indexer = createTestIndexer();
    const proxyAddress = "0xcccccccccccccccccccccccccccccccccccccccc" as const;
    const eoaOwner = "0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb" as const;
    ...
    const orderBefore = (await indexer.ConditionalOrder.getAll())[0]!;
    ...
    const orderAfter = (await indexer.ConditionalOrder.getAll())[0]!;
...
describe("Trade Handler", () => {
  ...
  it("should lowercase token addresses and owner", async () => {
    ...
    const trade = (await indexer.Trade.getAll())[0]!;
    expect(trade.owner).toBe("0xabcdef1234567890abcdef1234567890abcdef13");
  ...
  it("should link Trade to ConditionalOrder by owner", async () => {
    ...
    const orderId = (await indexer.ConditionalOrder.getAll())[0]!.id;
    const trade = (await indexer.Trade.getAll())[0]!;
  ...
  it("should resolve COWShed proxy realOwner on Trade", async () => {
    const indexer = createTestIndexer();
    const proxyAddress = "0xdddddddddddddddddddddddddddddddddddddddd" as const;
    const eoaOwner = "0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb" as const;
