...
/** Thrown by orderbookTerminalOrdersByUids when any requested UID is absent or
 *  non-terminal — thrown results are NOT cached, so only all-terminal batches
 *  persist. Callers fall back to the uncached orderbookOrdersByUids. */
export class NotAllTerminalError extends Error {
  constructor(requested: number, terminal: number) {
    super(`[COW:not-all-terminal] ${terminal}/${requested} terminal`);
    this.name = "NotAllTerminalError";
  }
}
...
/**
 * Cacheable by_uids variant for terminal-heavy batches (historical precompute,
 * stale-candidate drains). Terminal statuses never change, so an all-terminal
 * batch is safe to cache forever — the effect cache survives resyncs, unlike
 * the OrderUidCache entity, making repeat backfills skip these fetches
 * entirely. Any absent or still-open UID throws instead (uncached), and the
 * caller re-fetches via the fresh path.
 */
export const orderbookTerminalOrdersByUids = createEffect(
  {
    name: "orderbookTerminalOrdersByUids",
    input: S.schema({ chainId: S.number, uidsJson: S.string }),
    output: S.string, // JSON OrderbookOrder[] (slim: uid/status/amounts/validTo only)
    cache: true, // only all-terminal results ever return (others throw)
    rateLimit: { calls: 50, per: "second" as const },
  },
  async ({ input }): Promise<string> => {
    const apiBaseUrl = ORDERBOOK_API_URLS[input.chainId];
    const uids = JSON.parse(input.uidsJson) as string[];
    if (!apiBaseUrl) throw new NotAllTerminalError(uids.length, 0);
    const orders = await fetchOrdersByUidsRaw(apiBaseUrl, uids);
    const terminal = orders.filter((o) => TERMINAL.has(o.status));
    if (terminal.length !== uids.length) {
      throw new NotAllTerminalError(uids.length, terminal.length);
    }
    // Slim the cached payload — keep only what fetchOrderStatusByUids consumes.
